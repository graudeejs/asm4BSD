...
while (my $line = <MF>) {
	...
	if ($line =~ m/^\d+[ \t].*\{.*/ or $join_lines == 1) {	# lines we're interested start with syscall number
		...
		if ($line =~ m/^\d+[ \t]+\w+[ \t]/) {
			my ($number, $type, $name) = $line =~ m/(\d+)[ \t]+(\w+)[ \t]+([ \w]+)/;
			$name =~ tr/[a-z] /[A-Z]_/;
			my $prefix ="SYS_";
			$prefix = "" if ($name =~ /^SYS_/);
...
print "\n\n;==============================================\n";
print "; Converted with NetBSD_syscalls4fasm.pl v$version\n";
...
sub process_syscall_line() {
	...
	($number, $type, $comment, $name) = ($pline =~ m/^(\d+) (\w+).+\{ (.*\|sys\|\d*\|(\w+)\(.*\);) ??\}/);
	$name =~ tr/[a-z]/[A-Z]/;
	$comment =~ s/\|.*\|/ /;
